...
class RefundMailService
{
     ...
     *
     * @param Order $order
     * @param float $refundedAmount amount the Unzer confirmed
            ...
            $this->deliver(
                ...
                static function (Email $mailer) use ($order, $refundedAmount, $currency, $recipient): bool {
                    return $recipient === ModuleSettings::MAIL_RECIPIENT_OWNER
                        ? $mailer->sendUnzerRefundMailToOwner($order, $refundedAmount, $currency)
                        : $mailer->sendUnzerRefundMailToCustomer($order, $refundedAmount, $currency);
                }
            );
        }
    }
    ...
    public function sendCancelMail(Order $order, ?float $refundedAmount, string $currency): void
    {
        ...
        foreach ($recipients as $recipient) {
            $this->deliver(
                ...
                static function (Email $mailer) use ($order, $refundedAmount, $currency, $recipient): bool {
                    return $recipient === ModuleSettings::MAIL_RECIPIENT_OWNER
                        ? $mailer->sendUnzerCancelMailToOwner($order, $refundedAmount, $currency)
                        : $mailer->sendUnzerCancelMailToCustomer($order, $refundedAmount, $currency);
                }
            );
        }
    }
    ...
    protected function deliver(Order $order, string $recipient, string $type, callable $send): void
    {
        ...
        try {
            ...
            $this->log(
                ...
                sprintf(
                    'Unzer %s confirmation mail to %s for order %s: %s',
                    $type,
                    $recipientName,
            ...
            $this->log(
                LogLevel::ERROR,
                sprintf(
                    'Unzer %s confirmation mail to %s for order %s failed: %s',
                    $type,
                    $recipientName,
                    $this->orderNumber($order),
    ...
    protected function log(string $level, string $message, Order $order): void
    {
        try {
            ...
            $logger->log(
                sprintf('%s [%s] order %s', $message, $level, (string)$order->getId())
            );
        } catch (Throwable $throwable) {
