...
- Repair the Twig versions of `views/twig/frontend/tpl/order/unzer_installment.html.twig` and `unzer_sepa_secured.html.twig`, which had never been converted completely from Smarty and could not be compiled at all - a template cache warmup reported `Twig\Error\SyntaxError: Unclosed "(" ... at line 6` for both, so the installment secured and the SEPA direct debit secured payment methods were unusable in the checkout. The birthdate extraction carried mangled `regex_replace()` calls where the converter had mistaken the regex quantifiers for Twig delimiters and kept Smarty's colon argument syntax (`regex_replace("/^([0-9]{4)})[-]/":""`); they are replaced by the `|split('-')[n]` idiom already used by `unzer_installment_paylater.html.twig` and `unzer_invoice.html.twig`, which is the same thing for a `YYYY-MM-DD` value and carries no quantifiers for a future conversion run to break. Behind that first syntax error sat more of the same, none of which the warmup could report because Twig stops at the first one: the empty-birthdate comparison read `!= "0000-00 - 00"` (with spaces) and therefore never matched, so an empty birthdate ended up as `00`/`0000` instead of `0`; the month `<option>` line still contained a Smarty fragment plus, in the SEPA template, an empty `{% if %}`; `smarty.section.month.index` does not exist in Twig and is now the loop variable; the month loop ran `1..13` because Smarty's `{section start=1 loop=13}` is exclusive, giving a thirteenth month; and in the installment template the Unzer JS block still used the raw Smarty variables `[{$unzerpub}]`, `[{$unzerLocale}]`, `[{$totalgross}]`, `[{$uzrcurrency}]` and `[{$installrate}]`. All 23 Twig templates of the module compile now.
- `views/twig/frontend/tpl/order/unzer_installment_confirm.html.twig` included its two theme variants as `@unzer/unzer_installment_confirm_flow.html.twig` and `@unzer/unzer_installment_confirm_wave.html.twig`. There is no `@unzer` namespace - the module registers `@osc-unzer` - and the path inside it was missing as well, so the template failed to compile and the installment confirmation page could not be rendered. Both includes now use `@osc-unzer/frontend/tpl/order/...`, the same form every other template of the module uses. Found while recompiling all templates after the repair above; a warmup stops at the templates it is given, and this one is only reached through the confirmation step.
