...
[{capture assign="style"}]
    table.unzerrefund th, table.unzerrefund td {
...
[{include file="email/html/header.tpl" title="OSCUNZER_REFUND_MAIL_TITLE"|oxmultilangassign|cat:" #"|cat:$order->oxorder__oxordernr->value style=$style}]

    [{block name="unzer_email_html_refund_intro"}]
        <p>
            [{if $isUnzerOwnerMail}]
                [{oxmultilang ident="OSCUNZER_REFUND_MAIL_INTRO_OWNER"}]
            [{else}]
                [{oxmultilang ident="OSCUNZER_REFUND_MAIL_SALUTATION"}]
                [{$order->oxorder__oxbillfname->getRawValue()}] [{$order->oxorder__oxbilllname->getRawValue()}],
            [{/if}]
        </p>
        [{if !$isUnzerOwnerMail}]
            <p>[{oxmultilang ident="OSCUNZER_REFUND_MAIL_INTRO"}]</p>
        [{/if}]
    [{/block}]

    [{block name="unzer_email_html_refund_details"}]
        <table class="unzerrefund" border="0" cellspacing="0" cellpadding="0" width="100%">
            <tbody>
                <tr valign="top">
    ...
    [{block name="unzer_email_html_refund_note"}]
        [{if !$isUnzerOwnerMail}]
