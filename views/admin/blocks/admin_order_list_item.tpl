...
[{if $isUnzerPayment !== false}]
    ...
    <script type="text/javascript">
        ...
        unzer_order[0].textContent = "[{$listitem->oxorder__oxordernr->value}] [{if $listitem->oxorder__oxunzerordernr->value}] ([{oxmultilang ident='OSCUNZER_TRANSACTION_ORDERNR' suffix='COLON'}] [{$listitem->oxorder__oxunzerordernr->value}][{/if}]";
    </script>
[{else}]
    [{$smarty.block.parent}]
[{/if}]
